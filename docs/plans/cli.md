...
1. **Algorithmic over LLM whenever possible.** Default to deterministic algorithms; only call a model when no algorithmic path exists. New algorithmic call sites: slash command parsing (prefix match), context-window accounting (tokenizer + sum), MCP tool dispatch (name lookup + arg validation), tool selection for ambiguous intent (embedding similarity over tool descriptions, LLM fallback only), diff rendering, conversation truncation/compaction triggers, file-type classification, font picker filter, table layout. LLM stays the right tool for code generation, refactor, explain, prose summarization.
