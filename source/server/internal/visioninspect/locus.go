...
// mode: local/open vision is ALWAYS tried first (even under cloud_primary /
// cloud_only — a vision question is cheap grunt work that should stay local),
// and cloud vision is used only as a fallback and only when the locus permits
// crossing to cloud (every mode except open_only).
//
// This policy is deliberately NOT locus.Mode.Main()/Coproc(): those prefer
// cloud under cloud_primary. Vision-as-tool prefers local everywhere; the only
// thing the locus mode governs is whether cloud fallback is allowed at all.
//
//	mode         local first?   cloud fallback allowed?
//	cloud_only   yes            yes
...
// cloudAllowed reports whether the current locus permits crossing to cloud
// vision. Cloud fallback is allowed for every mode except open_only.
...
func (l *LocusInspector) Available() bool {
	...
	if l.local != nil && l.local.Available() {
		return true
	}
...
func (l *LocusInspector) Lookup(convID, imageID string) bool {
	...
	if l.local != nil && l.local.Lookup(convID, imageID) {
		return true
	}
...
func (l *LocusInspector) Inspect(ctx context.Context, convID, imageID, question string) (capabilities.VisionAnswer, error) {
	...
	localUsable := l.local != nil && l.local.Available()
