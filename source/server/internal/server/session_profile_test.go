...
func TestSessionProfile_SetGetAndLiveAccessor(t *testing.T) {
	...
	got, err := srv.GetSessionProfile(ctx, &proto.GetSessionProfileRequest{})
	if err != nil {
		t.Fatal(err)
	}
	...
	if srv.runnerDeps().Profiles().Restricts() {
		t.Fatal("default: runner accessor should be unrestricted")
	}

	// Enter planning.
	res, err := srv.SetSessionProfile(ctx, &proto.SetSessionProfileRequest{Name: "plan"})
	if err != nil || !res.GetOk() {
		t.Fatalf("SetSessionProfile(plan): err=%v resp=%+v", err, res)
	}
	got, _ = srv.GetSessionProfile(ctx, &proto.GetSessionProfileRequest{})
	...
	res, err = srv.SetSessionProfile(ctx, &proto.SetSessionProfileRequest{Name: "bogus"})
	if err != nil {
	...
	// Leave planning.
	res, _ = srv.SetSessionProfile(ctx, &proto.SetSessionProfileRequest{Name: "off"})
	if res.GetOk() {
	...
	res, _ = srv.SetSessionProfile(ctx, &proto.SetSessionProfileRequest{Name: "default"})
	if !res.GetOk() {
