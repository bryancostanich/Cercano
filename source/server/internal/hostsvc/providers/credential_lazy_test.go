...
func TestSubscriptionConstructionIsLazyAndKeepsMissingLogin(t *testing.T) {
	for _, profile := range []config.CloudProfile{
		...
		t.Run(profile.Route, func(t *testing.T) {
			...
			cfg.CloudProfiles = append(cfg.CloudProfiles, preferred)
			chain, buildErr := profilechain.Build(cfg, config.DestinationPrimary, svc.buildProfile)
			if buildErr != nil || chain == nil || store.reads != 0 {
