...
- [x] Confirm the exact profile/session-control path from planning mode:
  - profile broker;
                                                                                                                            ...
                                                                                                                            - mode chip rendering.
- [x] Confirm conversation store migration conventions and how `conversations.db` schema changes are handled.
- [x] Choose the ledger storage shape during implementation, biased toward the smallest durable structure that supports this effort:
  - active brief;
                                                                                                                            - brief revisions;
                                                                                                                            - run state;
                                                                                                                            - captured decisions;
                                                                                                                            - final review state.
- [x] Keep the ledger conversation-scoped. Do not introduce a global job manager.
  Expected tests:
...
- [x] Add an autonomous profile alongside the existing planning profile in the server profile broker.
- [x] Add autonomous posture text to the active profile context:
...
- [x] Extend session profile change plumbing so the CLI receives and stores `autonomous` just like `plan`.
- [x] Update `renderPermissionModeChip` to render `mode: autonomous | <permission-mode>`.
- [x] Keep planning chip behavior unchanged.
  Expected tests:
...
- [x] Add `suggest_autonomous` built-in:
...
- [x] Add `request_autonomous_exit` built-in:
...
- [x] Add CLI slash command `/auto [goal]`:
...
- [x] Extend CLI session-control prompt helpers:
...
- [x] Ensure `y/n/d/c` behavior matches planning mode:
...
- [x] Add durable ledger storage attached to conversation ID.
- [x] Store active run brief:
  - `goal`;
                                                                                                                            - `done_when`;
                                                                                                                            - `constraints`;
                                                                                                                            - `review_points`.
- [x] Store append-only brief revisions:
  - revision number;
                                                                                                                            - actor;
                                                                                                                            - timestamp;
                                                                                                                            - reason;
...
- [x] Store run state:
  - proposed;
                                                                                                                            ...
                                                                                                                            - abandoned;
                                                                                                                            - blocked, if needed.
- [x] Preserve optional source references for plan bridge:
  - source kind;
                                                                                                                            - plan path;
                                                                                                                            - spec path;
                                                                                                                            - source conversation ID if useful.
- [x] On reconnect/resume, restore enough state for the profile chip and final-review flow to be correct.
  Expected tests:
...
- [x] Add `capture_decision` built-in, available/encouraged in autonomous mode.
- [x] Require compact design-decision-protocol fields:
  - `decision_point`;
...
- [x] Persist decision entries in the autonomy ledger with sequence number and timestamp.
- [x] Emit a lightweight stream/progress event if there is an existing good event path; otherwise keep it ledger-only for V1.
- [x] Update autonomous profile text so meaningful forks require using the design-decision protocol and `capture_decision` before proceeding.
- [x] Make the protocol clear that ordinary low-level implementation choices do not need logging; meaningful forks do.
  Expected tests:
...
- [x] After successful `request_plan_approval`, allow the assistant to offer autonomous execution by calling `suggest_autonomous`.
- [x] Update planning/executing profile instructions to make this bridge explicit:
...
- [x] Avoid overloading `request_plan_approval`; it should continue to mean leaving planning mode and starting execution under human approval.
- [x] Ensure a plan-derived brief is still user-approved before autonomous mode starts.
  Expected tests:
...
- [x] Implement final-review state after `request_autonomous_exit` approval.
- [x] Present captured decisions one by one, in order, with enough detail for review:
  - decision point;
                                                                                                                            - chosen path;
...
- [x] When review completes, mark ledger completed and exit the autonomous profile.
- [x] If the user asks to revise a decision, keep autonomous mode active and have the agent update work; capture a new decision/revision if needed.
  Expected tests:
...
- [x] Add a built-in `autonomous-mode` or `autonomous-run` protocol document if the profile text should remain compact.
- [x] Ensure active autonomous posture says:
...
- [x] Add tests locking the high-risk threshold language so it does not drift into asking for every decision.
  Expected tests:
...
- [x] Run targeted server tests for profile, capabilities, conversation store, tool loop, and protocol catalog.
- [x] Run targeted CLI tests for slash command, confirmation prompts, status chip, and review UI.
- [x] Run module-level `go test ./...` in server and CLI if interfaces/protobuf/store schema changed broadly.
- [x] Manually exercise or add an integration test for:
  - `/auto <goal>`;
                                                                                                                            - approve brief;
                                                                                                                            - capture a decision;
                                                                                                                            - request exit;
