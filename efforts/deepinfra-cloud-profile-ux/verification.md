...
## Settings/client/CLI implementation (direct)

Added UpdateRoutingAssignments as an atomic routing-draft save: nil preserves, present-empty resets, invalid references/self-loops reject without partial mutation. Provider availability warnings are distinct from successful configuration persistence. GetCloudProviders returns assignments. Client codecs preserve choice-message presence, sparse maps, AWS metadata, and cloned drafts. Recommended quality models travel separately from effective choices so resetting an override does not display the former override as the inherited recommendation.

Cloud UI now exposes all four bindings with None options, chat/dispatch destination and quality defaults, sparse quality/image choices, custom IDs, explicit Save/Discard, and unsaved row/tab/close confirmation. Model picker cancellation is covered. Authentication remains separate. Models is now Local Models; hosted entries are excluded from runtime/catalog candidates, download selection, and RAM-estimate keys. Existing server defensive download guards remain untouched.
