...
Fallback authorization applies only to the interrupted request, or its explicitly authorized fresh retry after reconnect. Here, a request is the owning user turn: its later inference iterations may use the selected route, but the next user turn starts with configured routing. The switch itself occurs at the interrupted inference boundary, never by replaying completed tools. It does not modify active profiles, default routing, permission settings, or authorization for subsequent unrelated requests. Only configured, policy-permitted fallback is offered. An authentication failure on the selected fallback is not permission to silently move to yet another provider.
