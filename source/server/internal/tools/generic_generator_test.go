...
func TestGenericGenerator_Generate_ConstructsPrompt(t *testing.T) {
	spy := &SpyProvider{}
	handler := tools.NewGenericGenerator(spy)

	inputCode := "func Add(a, b int) int { return a + b }"
	instruction := "Refactor this code"
	ctx := context.Background()

	_, err := handler.Generate(ctx, instruction, inputCode)
	if err != nil {
	...
	if !strings.Contains(prompt, instruction) {
		t.Errorf("Prompt should contain the instruction")
	}
}

func TestGenericGenerator_Generate_HandlesError(t *testing.T) {
	expectedErr := context.DeadlineExceeded
	spy := &SpyProvider{Err: expectedErr}
	handler := tools.NewGenericGenerator(spy)

	_, err := handler.Generate(context.Background(), "instruction", "func foo()")
...
func TestGenericGenerator_Fix_ConstructsPrompt(t *testing.T) {
	spy := &SpyProvider{}
	handler := tools.NewGenericGenerator(spy)
