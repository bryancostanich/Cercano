...
func TestDragScroll_ContinuesWhileHeldAtEdge(t *testing.T) {
	...
	m.viewport.SetYOffset(m.viewport.TotalLineCount()) // bottom
	...
	afterMotion := m.viewport.YOffset()
	...
	if m.viewport.YOffset() >= afterMotion {
		t.Fatalf("ticks did not keep scrolling: %d -> %d", afterMotion, m.viewport.YOffset())
	}
