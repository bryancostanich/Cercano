...
OUT="$ROOT/efforts/llama-server-memory-guard/restart-verify/run.log"
CLIENT_GO="$ROOT/efforts/llama-server-memory-guard/restart-verify/start_glm_client.go"
AGENT_BIN="/Users/bryancostanich/bin/.cercano-libexec/cercano"
PORT="50052"
...
run "ps -Ao pid,ppid,pgid,rss,comm,args | grep -i '[l]lama-server' || true"

OLD_AGENT=$(ps -Ao pid,args | awk '/[.]cercano-libexec\/cercano agent/ {print $1; exit}')
if [[ -z "${OLD_AGENT:-}" ]]; then
  log "no active libexec agent found; aborting"
  exit 1
fi
log "old_agent=$OLD_AGENT"

log "sending SIGTERM to old agent $OLD_AGENT"
kill -TERM "$OLD_AGENT" || true

# Sample quickly through the dangerous window.
log "sampling llama-server processes during shutdown window"
for i in $(seq 1 40); do
  printf 'sample=%02d ' "$i" | tee -a "$OUT"
  ps -Ao pid,ppid,pgid,rss,args | grep -i '[l]lama-server' | tee -a "$OUT" || printf 'no llama-server\n' | tee -a "$OUT"
  sleep 0.25
done

# If the CLI reconnect loop hasn't relaunched the agent, start it ourselves.
if ! nc -z 127.0.0.1 "$PORT" >/dev/null 2>&1; then
  log "port $PORT not listening; starting replacement agent manually"
  nohup "$AGENT_BIN" agent >> "$OUT.agent.stdout" 2>&1 &
  echo $! > "$ROOT/efforts/llama-server-memory-guard/restart-verify/new-agent.pid"
fi
...
log "summary counts"
LLAMA_COUNT=$(ps -Ao args | grep -i '[l]lama-server' | wc -l | tr -d ' ')
log "final_llama_count=$LLAMA_COUNT"
if [[ "$LLAMA_COUNT" -gt 1 ]]; then
  log "FAIL: more than one llama-server remains"
  exit 2
fi
log "PASS: restart verifier completed without overlapping final llama-server"
