...
Execute the approved spec.md. Preserve legacy history without migration; all new reporting starts at a persisted cutover. Accounting must be asynchronous and must never wait on storage or transport on the inference path. No implementation has begun. The user approved the spec in conversation; this execution plan requires separate approval.
