...
// grepCap searches across a directory for a pattern. Prefers ripgrep (rg)
...
func (grepCap) Execute(ctx context.Context, call *capabilities.Call) (*capabilities.Result, error) {
	...
	return grepRunGrep(ctx, a)
}

func grepRunRipgrep(ctx context.Context, a grepArgs) (*capabilities.Result, error) {
	...
	if err != nil {
		...
		if errors.As(err, &ee) && ee.ExitCode() == 1 {
			res := capabilities.NewRowsResult(nil)
			res.Detail = "0 matches"
			return res, nil
		}
		return nil, fmt.Errorf("grep (rg): %w: %s", err, string(bytes.TrimSpace(out)))
	}
	rows := grepParseVimgrep(string(out))
	r := capabilities.NewRowsResult(rows)
...
func grepRunGrep(ctx context.Context, a grepArgs) (*capabilities.Result, error) {
	args := []string{"-R", "-I", "-n", "-H"}
	if a.CaseInsensitive {
		args = append(args, "-i")
	}
	...
	rows := grepParseGrepNH(string(out))
	r := capabilities.NewRowsResult(rows)
	r.Detail = countLabel(len(rows), "match", "matches")
	if r.Note == "" {
...
// grepParseVimgrep parses rg --vimgrep output: path:line:col:content.
func grepParseVimgrep(text string) []map[string]any {
...
// grepParseGrepNH parses grep -nH output: path:line:content.
func grepParseGrepNH(text string) []map[string]any {
	var rows []map[string]any
	scanner := bufio.NewScanner(strings.NewReader(text))
	scanner.Buffer(make([]byte, 64*1024), 1024*1024)
	for scanner.Scan() {
