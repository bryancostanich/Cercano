...
func TestWorkerReasoningDiagnosticAuthenticatedNoFallback(t *testing.T) {
	...
	zero := 0.0
	toolSvc := buildWorkerToolSvcWithDiagnostic(nil, nil, nil, resolver.Cloud(), resolver.Open(), c, nil, nil, nil, nil, resolver.(reasoningexperiment.Service), nil, resolver.Candidates)
