# Co-processor documentation — deprecated for now

> **Historical reference, not current standalone setup instructions.**
> External co-processor mode is deprecated for now. New users should start with
> the [standalone agent](../agent/README.md). This snapshot may contain obsolete
> architecture, installation, roadmap, and cost claims. It preserves the previous
> root README, with relative links adjusted for its new location.
>
> This documentation migration removes no runtime functionality and sets no
> removal date. See the [deprecation overview](README.md) for scope.
...
For a detailed guide on writing custom SKILL.md files, see `docs/agent-skills-guide.md` (historical reference; this file is not present in the repository).
