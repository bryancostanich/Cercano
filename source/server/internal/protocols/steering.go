...
- Delegate mechanical, well-specified work. Multi-file edits with a clear spec, bulk renames, systematic audits, and research sweeps that would flood your context belong in a sub-agent: call the dispatch tool with the task and the tools it needs (e.g. ["Read", "Grep", "Edit", "RunCommand"]); it runs its own read-edit-build-test loop and returns once with the result.`
