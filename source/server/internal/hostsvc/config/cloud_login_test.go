	...
	for _, profile := range []cfg.CloudProfile{
		...
		for _, active := range []string{"named", "other"} {
			t.Run(profile.Route+"/"+active, func(t *testing.T) {
				...
				if !result.Reauthenticated || result.Profile != profile || !reflect.DeepEqual(before, service.Get()) {
...
func TestInitialLoginStillCreatesAndActivates(t *testing.T) {
	...
	got := service.Get()
	if !result.Active || result.Reauthenticated || got.ActiveCloudProfile != "new" || got.BackupCloudProfile != "previous" {
