...
func TestBuildThemeSections(t *testing.T) {
	...
	for _, want := range []string{"Theme", "Theme · Chrome", "Theme · Content", "Theme · Semantic", "Theme · Actions"} {
		if !titles[want] {
			t.Errorf("missing section %q", want)
		}
	}
	for _, want := range []string{"theme-select", "color:accent", "color:buffer_code", "color:selection_bg", "color:activity_base", "theme-save", "theme-save-as", "theme-delete", "theme-import"} {
		if !keys[want] {
