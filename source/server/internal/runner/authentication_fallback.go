...
func (s *authenticationFallbackStream) Next() (llm.StreamEvent, bool, error) {
	...
	if ok && event.Type == llm.EventMessageStart && s.owner.selected && event.Route == nil {
		route := s.owner.selectedRoute()
		event.Route = &route
	}
...
func (p *authenticationFallback) selectedRoute() llm.ServingRoute {
	return llm.ServingRoute{Provider: p.fallback.Name(), Destination: "local", Model: p.model, ContextWindow: p.window, ContextWindowKnown: p.windowKnown}
}
func (p *authenticationFallback) TargetForContext(ctx context.Context, req inference.Call) llm.ServingRoute {
	if p.selected {
		return p.selectedRoute()
	}
	return inference.TargetForContext(ctx, p.primary, req)
}
func (p *authenticationFallback) TargetForCall(req inference.Call) llm.ServingRoute {
	if p.selected {
		return p.selectedRoute()
	}
	return inference.TargetForCall(p.primary, req)
}
