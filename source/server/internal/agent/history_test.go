...
func TestBuildLLMHistory_TextOnly(t *testing.T) {
	...
	if got[0].Role != llm.RoleUser || got[0].Blocks[0].Text != "hi" {
		t.Errorf("turn0 = %+v", got[0])
	}
	if got[1].Role != llm.RoleAssistant || got[1].Blocks[0].Text != "yo" {
...
func TestBuildLLMHistory_ToolRoundTripPreserved(t *testing.T) {
	...
	if got[1].Blocks[0].Type != llm.BlockToolUse || got[1].Blocks[0].ToolUseID != "u1" {
		t.Errorf("tool_use lost: %+v", got[1])
	}
	if got[2].Blocks[0].Type != llm.BlockToolResult || got[2].Blocks[0].ToolUseRef != "u1" {
		t.Errorf("tool_result lost: %+v", got[2])
	}
}
...
func TestBuildLLMHistory_OrphanToolUseStripped(t *testing.T) {
	...
	got := BuildLLMHistory(turns)
	if len(got) != 1 {
		t.Fatalf("len = %d, want 1 (orphan tool_use message dropped)", len(got))
	}
	if got[0].Blocks[0].Text != "list" {
		t.Errorf("kept wrong message: %+v", got[0])
	}
}
...
func TestBuildLLMHistory_OrphanToolResultStripped(t *testing.T) {
	...
	got := BuildLLMHistory(turns)
	if len(got) != 1 {
		t.Fatalf("len = %d, want 1 (orphan tool_result dropped)", len(got))
	}
	if got[0].Blocks[0].Text != "hi" {
		t.Errorf("kept wrong message: %+v", got[0])
	}
}
