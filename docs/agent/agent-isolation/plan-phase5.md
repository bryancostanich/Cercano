...
4. **Credentials are proxied on-demand over the stream, NOT baked into the snapshot (REVISED per user).** The worker requests a credential/token from the host over the existing bidi stream whenever its provider needs one — exactly like the permission round-trip. The host owns the live credential + its refresh (keychain for API keys, the ChatGPT OAuth source for subscription tokens) and returns a fresh value; on mid-turn expiry (a long turn) the worker's token source re-asks and the host hands back a refreshed token. This dissolves auth-backend special-casing (API key, ChatGPT-subscription, etc. are all "ask the host") and removes the static-token-expiry edge. Wire: `WorkerToHost.CredentialRequest{profile}` ↔ `HostToWorker.CredentialResponse{token, error}`. The old `ConfigSnapshot.resolved_credential` field is deprecated/unused in favor of this. (Supersedes the earlier snapshot-credential approach — the worker never holds a durable credential.)
...
- [ ] **Step 4: Default = worker (USER-DECIDED).** Production default `ExecutionMode` = `worker` — the whole point of this work is crash isolation, so the isolated path is the default; in-process is for embedded mode / tests. WIRING NUANCE this creates: the existing server test suite constructs `Server` and must NOT silently start spawning worker processes (slow + needs the built binary). So: (a) the PRODUCTION config default (the value `config.Defaults()` / the real config load produces) is `worker`; (b) test construction paths explicitly select `in_process` (set the field in the test's config, or the test harness default) so the existing suite stays fast + hermetic; (c) the both-modes tests (Step 2) and the crash proof (Step 3) explicitly select `worker`. Grep how tests build their config/Server and make the in-process selection explicit there — do not make `worker` unreachable, and do not make the existing suite spawn processes.
