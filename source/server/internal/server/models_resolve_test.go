...
func TestGetConfig_ModelTiersIncludesVision(t *testing.T) {
	s, _ := newTestServer()
	s.cfgSvc.Mutate(func(c *config.Config) {
		c.Models.SetOverride(c.OpenRuntime, config.TierVision, "gemma-3-4b-it-q4_k_m")
	})
	...
	if gc.ModelTiers["llama_server.vision"] != "llama_server:catalog:gemma-3-4b-it-q4_k_m" {
