...
Worker delivery groundwork: the collector now supports atomic worker-batch admission with a persistence receipt. Capacity/malformed rejection accepts no records and does not count worker-retained data as host loss. A receipt only succeeds after all observations persist, including batches split across writes; failed retries and bounded shutdown return negative receipts. Tests cover caller mutation isolation, no premature acknowledgment, rejection ownership, and shutdown uncertainty.

Added typed optional-count worker protocol messages and bounded conversion helpers (64 observations, 1 MiB ceiling, semantic validation, all-or-nothing decode). Round trips test every metadata field, explicit zero versus unknown, UTC conversion including the Unix epoch, and malformed batches. Bindings regenerated using source/proto/generate.sh. `go test -race ./internal/worker ./internal/telemetry` and command/proto tests pass. These new protocol messages are not wired into live delivery yet.

Lifecycle finding before live wiring: workerRunner captures TurnDone but waits for EOF before returning; worker closes its sender before sending terminal output/EOF. Waiting for accounting receipts before sender closure would therefore block turn completion on storage. A per-turn accounting queue alone does not establish an independent background delivery lifetime. Further transport work must preserve non-blocking turn completion and reuse the existing gRPC connection without changing permission/control semantics. No accounting wait has been introduced into live turns.


Independent stream decision and proof: use an accounting-only RPC on the existing worker gRPC connection, not a post-terminal extension of the permission/tool stream. Recorded the alternatives and trade-offs in the autonomy ledger. Added typed empty-message readiness handshake, bounded single-frame send slot, stable content-derived batch IDs, matching persistence receipts, and disconnect cancellation. No new connection/dependency or tool permission capability is introduced. Unused same-turn batch/receipt envelope fields were removed before live integration.

A real in-memory gRPC test holds an accounting receipt open while an ordinary turn RPC reaches its handler and returns the expected InvalidArgument for a malformed start. This proves RPC isolation, not successful inference completion. Other fixtures verify stable IDs on retry, unrelated receipt rejection, and writer release on disconnect. These tests pass ten race-enabled repetitions. Full worker and telemetry race tests plus command tests pass after regeneration. Production source binding, host attachment, process shutdown/drain, and worker health transport are still pending; no accounting wait was inserted into the existing RunTurn flow.
