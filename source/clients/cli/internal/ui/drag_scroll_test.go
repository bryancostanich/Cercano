...
func TestDragScroll_ContinuesWhileHeldAtEdge(t *testing.T) {
	...
	m.mainChat().SetYOffset(m.mainChat().TotalLineCount()) // bottom
	startOffset := m.mainChat().YOffset()
	if startOffset == 0 {
	...
	m.mainChat().selection = textSelection{Active: true, Dragging: true,
	...
	m = next.(Model)
	if !m.mainChat().DragScrolling() || cmd == nil {
		t.Fatalf("expected edge auto-scroll to start (dragScrolling=%v, cmd=%v)", m.mainChat().DragScrolling(), cmd != nil)
	}
	afterMotion := m.mainChat().YOffset()
	...
	if m.mainChat().YOffset() >= afterMotion {
		t.Fatalf("ticks did not keep scrolling: %d -> %d", afterMotion, m.mainChat().YOffset())
	}

	// Releasing stops the loop.
	next, _ = m.Update(tea.MouseReleaseMsg(tea.Mouse{X: 5, Y: m.scrollbarTop - 1}))
	m = next.(Model)
	if m.mainChat().DragScrolling() {
