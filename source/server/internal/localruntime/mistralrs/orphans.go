...
// pidRegistry records the mistral.rs processes this cercano process has
...
// shared file would race, and a name-based hunt (pgrep mistralrs) could
...
func defaultRegistryDir() string {
	...
	return filepath.Join(home, ".cercano", "run", "mistralrs")
}
...
// SweepOrphans reaps mistral.rs processes left behind by cercano processes
// that died without cleanup. Call it once at startup, before spawning
// anything. Live owners' files are left strictly alone; a server is only
// killed when its recorded PID still runs the recorded binary (and model,
// when recorded), so a recycled PID can never take down an unrelated
// process.
func (p *Provider) SweepOrphans(sink localruntime.LogSink) {
	...
	for _, dirEntry := range entries {
		...
		for _, server := range state.Servers {
			...
			p.emit(sink, "warn", "", "",
				fmt.Sprintf("reaped orphaned mistral.rs pid %d (model %s, owner pid %d died without cleanup)",
...
// liveSiblingFor returns a healthy mistral.rs process for modelPath that is
...
func (p *Provider) liveSiblingFor(ctx context.Context, modelPath, binary string, sink localruntime.LogSink) (*liveSibling, bool) {
	...
	for _, dirEntry := range entries {
		...
		for _, server := range state.Servers {
			...
			cancel()
			if err != nil {
				p.emit(sink, "warn", "", "", fmt.Sprintf("ignored mistral.rs sibling pid %d from owner pid %d: health failed: %v", server.PID, state.OwnerPID, err))
				continue
			}
