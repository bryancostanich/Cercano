...
func TestChatView_ProgressAfterToolStartIsDurableSystemLine(t *testing.T) {
	...
	c.Apply(chatProgressMsg{note: "autonomous: running Bash"})
	...
	if c.entries[1].Role != RoleSystem || !strings.Contains(c.entries[1].Content, "autonomous: running Bash") {
		t.Fatalf("second entry should be durable autonomous progress, got %+v", c.entries[1])
	}
}
